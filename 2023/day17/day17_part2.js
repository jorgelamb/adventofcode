  ...
  if(isValid(row, column)) {
    ...
    if(length<=10) {
      ...
      if(existing.length>0 || existingProcessed.length>0) {
        ...
        if(length>=4 && row==map.length-1 && column==map[0].length-1) {
          log(`Found at ${newHeatTotal}`);
          throw `Found at ${newHeat} ${newHeat.reduce((a, b) => a+b, 0)} ${newDirections}`;
        }
...
addIfValid(0, 1, "E", 1, 0, [], "E");
addIfValid(1, 0, "S", 1, 0, [], "S");
