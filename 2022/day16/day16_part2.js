...
function getBest(valvesToConsider) {
  if(cache[valvesToConsider]) {
    return cache[valvesToConsider];
  }
  var pending = ["0_AA"];
  var best = {};
  best["0_AA"] = 0;
  ...
  while(pending.length>0) {
    ...
    var pendingValves = valvesToConsider.split(",").filter(v => currentOpenValves.indexOf(v)<0);
  ...
  cache[valvesToConsider] = max;
  return max;
}

var max = 0;
var countValvesWithFlow = allValvesWithFlow.split(",").length;
for(var split=0; split < (1<<(countValvesWithFlow-1)); split++) {
  var myValves = allValvesWithFlow.split(",").filter((v, idx) => (split & (1<<idx))>0).join(",");
  var elephantValves = allValvesWithFlow.split(",").filter((v, idx) => (split & (1<<idx))==0).join(",");
  var result = getBest(myValves) + getBest(elephantValves);
  if(result > max) {
    console.log(result);
    max = result;
  }
