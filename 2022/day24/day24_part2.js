...
function fromTo(fromRow, fromColumn, toRow, toColumn) {

var start = [fromRow, fromColumn];
var goal = [toRow, toColumn];
  ...
  while(pending.length>0) {
    ...
    [[0, 1], [0, -1], [1, 0], [-1, 0], [0, 0]].forEach(d => {
      ...
      if(newRow == goal[0] && newColumn == goal[1]) {
        found = true;
      } else if(newRow>=0 && newColumn>=0 && newRow<HEIGHT && newColumn<WIDTH && map[newRow][newColumn] != "#") {
...
var result1 = fromTo(0, 1, map.length-1, map[0].length-2);
var result2 = fromTo(map.length-1, map[0].length-2, 0, 1);
var result3 = fromTo(0, 1, map.length-1, map[0].length-2);

log(`result: ${result1+result2+result3}`);
