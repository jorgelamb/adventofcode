...
//console.log(hashes);
console.log(total);
console.log(Object.entries(seaMonsterMarks).length);
//console.log(JSON.stringify(seaMonsterMarks));
log("roughness: "+(hashes-Object.entries(seaMonsterMarks).length));



var fullTileSize = tiles[0].lines.length;
var fullImageWidth = (maxX-minX+1)*fullTileSize;
var fullImageHeigth = (maxY-minY+1)*fullTileSize;
var fullDebug = "";
for(var j=fullImageHeigth-1; j>=0; j--) {
  fullDebug += "\n";
  for(var i=0; i<fullImageWidth; i++) {
    fullDebug += getDebugPixel(i, j);
  }
}
console.error("FULL DEBUG "+fullDebug);
console.debug(fullDebug);

var tilesDebug = "";
for(j=maxY; j>=minY; j--) {
  for(var i=minX; i<=maxX; i++) {
    tilesDebug += map[i+"_"+j].id+" "+map[i+"_"+j].rotation+"/"+map[i+"_"+j].flipped+"\t";
  }
  tilesDebug += "\n";
}
console.error(tilesDebug);

function getDebugPixel(x, y) {
  var tile = map[(Math.floor(x/fullTileSize)+minX)+"_"+(Math.floor(y/fullTileSize)+minY)];
  var offsetX = (x % fullTileSize);
  var offsetY = (y % fullTileSize);
...
console.log(JSON.stringify(tiles.filter(t => t.id=="1409")));
